...
namespace gpu {
namespace glsl {
const char draw_clockwise_atomic_path[] = R"===(/*
 * Copyright 2023 Rive
 */
...
VERTEX_MAIN(_EXPORTED_drawVertexMain, Attrs, attrs, _vertexID, _instanceID)
{
    ...
    {
        uint4 coverageData =
            STORAGE_BUFFER_LOAD4(_EXPORTED_pathBuffer, pathID * 4u + 3u);
        v_pathID = pathID;
...
VERTEX_MAIN(_EXPORTED_drawVertexMain, Attrs, attrs, _vertexID, _instanceID)
{
    ...
    uint4 coverageData = STORAGE_BUFFER_LOAD4(_EXPORTED_pathBuffer, pathID * 4u + 3u);
    v_coveragePlacement = coverageData.xy;
    v_coverageCoord = vertexPosition + uintBitsToFloat(coverageData.zw);
#endif
...
#ifdef _EXPORTED_BORROWED_COVERAGE_PASS
...
FRAG_DATA_MAIN(half4, _EXPORTED_drawFragmentMain)
{
...
#elif defined(_EXPORTED_ATLAS_BLIT)
         ...
         // IMAGE_PAINT_TYPE
    {
        ...
        else
        {
            ...
            paintColor = TEXTURE_SAMPLE_LOD(_EXPORTED_imageTexture,
                                            imageSampler,
                                            paintCoord,
                                            lod);
            paintColor =
...
#ifdef _EXPORTED_ATLAS_BLIT
    paintColor.w *= filter_feather_atlas(
        v_atlasCoord,
    ...
    if (is_stroke(v_coverages))
    {
        ...
        apply_stroke_coverage(paintColor.w, fragCoverage, coverageIndex);
    }
    else // It's a fill.
#endif // !DRAW_INTERIOR_TRIANGLES
    {
        ...
        fragCoverage = clamp(fragCoverage, .0, 1.);
#endif
        apply_fill_coverage(paintColor.w, fragCoverage, coverageIndex);
    }
#endif // @ATLAS_BLIT
