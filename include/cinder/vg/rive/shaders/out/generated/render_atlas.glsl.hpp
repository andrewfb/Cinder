...
void main()
{
    float coverage = uintBitsToFloat(_fragCoverage.x);
    coverage += eval_feathered_fill(v_coverages);
    _fragCoverage.x = floatBitsToUint(coverage);
}
#endif
...
void main()
{
    float coverage = uintBitsToFloat(pixelLocalLoadANGLE(_plsCoverage).x);
    coverage += eval_feathered_fill(v_coverages);
...
#elif defined(_EXPORTED_ATLAS_RENDER_TARGET_RGBA8_UNORM)
...
#ifdef _EXPORTED_ATLAS_FEATHERED_FILL
FRAG_DATA_MAIN(half4, _EXPORTED_atlasFillFragmentMain)
{
...
#ifdef _EXPORTED_ATLAS_FEATHERED_STROKE
FRAG_DATA_MAIN(half4, _EXPORTED_atlasStrokeFragmentMain)
{
    VARYING_UNPACK(v_coverages, float4);
