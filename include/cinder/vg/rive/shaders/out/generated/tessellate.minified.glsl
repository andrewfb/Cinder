...
ATTR(
    0,
    float4,
    _EXPORTED_a_p0p1_); // End in '_' because D3D interprets the '1' as a semantic index.
...
ATTR(3, uint, _EXPORTED_a_x0x1);
ATTR(4, uint, _EXPORTED_a_reflectionX0X1);
ATTR(5, uint, _EXPORTED_a_segmentCounts);
ATTR(6, uint, _EXPORTED_a_contourIDWithFlags);
#else
ATTR(3,
     uint4,
     _EXPORTED_a_args); // [x0x1, reflectionX0X1, segmentCounts, contourIDWithFlags]
#endif
ATTR_BLOCK_END
#endif
...
VERTEX_MAIN(_EXPORTED_tessellateVertexMain, Attrs, attrs, _vertexID, _instanceID)
{
...
#ifdef SPLIT_UINT4_ATTRIBUTES
    ATTR_UNPACK(_instanceID, attrs, _EXPORTED_a_x0x1, uint);
    ATTR_UNPACK(_instanceID, attrs, _EXPORTED_a_reflectionX0X1, uint);
    ATTR_UNPACK(_instanceID, attrs, _EXPORTED_a_segmentCounts, uint);
    ATTR_UNPACK(_instanceID, attrs, _EXPORTED_a_contourIDWithFlags, uint);

    uint4 _EXPORTED_a_args = uint4(_EXPORTED_a_x0x1,
                          _EXPORTED_a_reflectionX0X1,
                          _EXPORTED_a_segmentCounts,
                          _EXPORTED_a_contourIDWithFlags);
#else
    ...
    uint pathID =
        contourID > 0u
            ...
            ? STORAGE_BUFFER_LOAD4(_EXPORTED_contourBuffer, max(contourID, 1u) - 1u).z
            : 0u;
...
FRAG_DATA_MAIN(TESSDATA4, _EXPORTED_tessellateFragmentMain)
{
