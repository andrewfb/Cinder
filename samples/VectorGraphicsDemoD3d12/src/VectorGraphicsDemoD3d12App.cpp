...
class BlendModesDemo : public Demo {
  public:
	...
	void setup( vg::CanvasD3d12Ref canvas ) override { mFont = Font( "Arial", 11 ); }
...
void VectorGraphicsDemoD3d12App::setup()
{
	...
	mDemos = { std::make_shared<PrimitivesDemo>(), std::make_shared<GradientsDemo>(), std::make_shared<TransformsDemo>(), std::make_shared<FeatheringDemo>(), std::make_shared<InstancingDemo>(), std::make_shared<BlendModesDemo>(),
		std::make_shared<ClippingDemo>(), std::make_shared<ImagesDemo>() };
	for( auto& d : mDemos )
		d->setup( mCanvas );
	switchDemo( 0 );
...
void VectorGraphicsDemoD3d12App::renderImGui( ID3D12GraphicsCommandList* cmdList )
{
	if( ID3D12DescriptorHeap* imguiHeap = ImGui::GetD3D12SrvHeap() ) {
		...
		// Demo selector
		const char* demoNames[] = { "Primitives", "Gradients", "Transforms", "Feathering", "Instancing", "Blend Modes", "Clipping", "Images" };
...
void VectorGraphicsDemoD3d12App::keyDown( KeyEvent event )
{
	if( event.getCode() == KeyEvent::KEY_ESCAPE || event.getChar() == 'q' )
		quit();
	else if( event.getChar() >= '1' && event.getChar() <= '8' )
...
CINDER_APP( VectorGraphicsDemoD3d12App, RendererD3d12( RendererD3d12::Options().debugLayer( true ) ), []( App::Settings* settings ) {
