                                ...
                                const char* shaderTypeDefine)
{
    ...
    std::ostringstream s;
    // With minified shaders, all identifiers are already minified (e.g., ushort -> V, INLINE -> e).
    // hlsl.glsl.hpp provides mappings from minified names to HLSL types (d -> half, V -> min16uint, etc.).
    // We only need to set the shader type define and feature flags.
