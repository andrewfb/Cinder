...
// glInvalidateFramebuffer is GL 4.3+/ES 3.0+. On desktop GL 4.1 (macOS), skip it.
// It's just an optimization hint, not required for correctness.
    ...
    gpu::ShaderMiscFlags shaderMiscFlags)
{
    ...
    // For MSAA mode (m_plsImpl == nullptr), don't push any PLS impl define.
    // The shader fallback in glsl.glsl.hpp will define D1 as A0 (NO_PERSPECTIVE)
    // which is what draw_path_vert expects for varying qualifiers.
    if (interlockMode == gpu::InterlockMode::atomics)
    {
    ...
    const ContextOptions& contextOptions)
{
    ...
    {
        // macOS only provides GL 4.1, but we can work with that using
        // fallback paths (no PLS, texel buffers instead of SSBOs).
        // Windows/Linux typically provide GL 4.3+.
        if (!capabilities.isContextVersionAtLeast(4, 1))
        {
            fprintf(stderr,
